...
function environment_alg(
        ::InfiniteStyle, ::HamiltonianStyle, below::AbstractMPS, ::AbstractMPO, above::AbstractMPS;
        tol = Defaults.tol, maxiter = Defaults.maxiter, krylovdim = Defaults.krylovdim,
...
function rightenv(ca::AbstractMPSEnvironments, ind, state::AbstractMPS)
    return rightenv(GeometryStyle(ca) & GeometryStyle(state), ca, ind, state)
end

function leftenv(ca::AbstractMPSEnvironments, ind, state::AbstractMPS)
    return leftenv(GeometryStyle(ca) & GeometryStyle(state), ca, ind, state)
end
...
function recalculate!(envs::AbstractMPSEnvironments,
    ...
    operator_style = OperatorStyle(operator)
    return recalculate!(
        geometry_style, operator_style, envs, below, operator, above; kwargs...
    )
end
...
function TensorKit.normalize!(
    ...
    return TensorKit.normalize!(
        geometry_style, operator_style, envs, below, operator, above
    )
end