...
function compute_leftenvs!(
    ...
    for i in 2:length(operator)
        envs.GLs[i] = envs.GLs[i - 1] *
            TransferMatrix(above.AL[i - 1], operator[i - 1], below.AL[i - 1], backend, allocator)
    end
    return λ, envs
end
...
function compute_rightenvs!(
        ...
        above::InfiniteMPS, alg, backend::AbstractBackend = DefaultBackend(), allocator = BufferAllocator()
    )
    # compute eigenvector
    ...
    for i in reverse(1:(length(operator) - 1))
        envs.GRs[i] = TransferMatrix(
            ...
            below.AR[i + 1], backend, allocator
        ) * envs.GRs[i + 1]
    end
    return λ, envs
end
...
function left_cyclethrough!(
    ...
    for site in eachindex(GL)
        GL[site + 1][index] = GL[site][leftinds] * TransferMatrix(
            above.AL[site], H[site][leftinds, 1, 1, index], below.AL[site], backend, allocator
        )
    end
    return GL
end
...
function right_cyclethrough!(
        ...
        above::InfiniteMPS = below, backend::AbstractBackend = DefaultBackend(), allocator = BufferAllocator()
    )
    # TODO: efficient transfer matrix slicing for large unitcells
...
function transfer_leftenv!(envs::InfiniteEnvironments, below, operator, above, site::Int, backend::AbstractBackend = DefaultBackend(), allocator = DefaultAllocator())
    T = TransferMatrix(above.AL[site - 1], operator[site - 1], below.AL[site - 1], backend, allocator)
    envs.GLs[site] = envs.GLs[site - 1] * T
    return envs
end

function transfer_rightenv!(envs::InfiniteEnvironments, below, operator, above, site::Int, backend::AbstractBackend = DefaultBackend(), allocator = DefaultAllocator())
    T = TransferMatrix(above.AR[site + 1], operator[site + 1], below.AR[site + 1], backend, allocator)
