...
GeometryStyle(::Type{InfiniteEnvironments}) = InfiniteStyle()

leftenv(::InfiniteStyle, envs::AbstractMPSEnvironments, site::Int, state) = envs.GLs[site]
rightenv(::InfiniteStyle, envs::AbstractMPSEnvironments, site::Int, state) = envs.GRs[site]
...
function issamespace(envs::AbstractMPSEnvironments,
    ...
    geometry_style = GeometryStyle(envs) & GeometryStyle(below)
        & GeometryStyle(operator) & GeometryStyle(above)
    return issamespace(geometry_style, OperatorStyle(operator), envs, below, operator, above)
end
function issamespace(
        ::InfiniteStyle, ::OperatorStyle, envs::AbstractMPSEnvironments,
        below::InfiniteMPS, operator::AbstractMPO, above::InfiniteMPS
    )
...
function recalculate!(::InfiniteStyle, ::OperatorStyle,
...
function initialize_environments(
    ...
    return initialize_environments(GeometryStyle(below) &
        GeometryStyle(operator) & GeometryStyle(above),
        OperatorStyle(operator),
        below, operator, above)
end
function initialize_environments(
        ::InfiniteStyle, ::MPOStyle,
        below::AbstractMPS, operator::AbstractMPO, above::AbstractMPS = below
    )
    L = check_length(below, operator, above)
...
function compute_leftenvs!(
    ...
    return compute_leftenvs!(geometry_style, operator_style, envs, below, operator, above, alg)
end

function compute_leftenvs!(::InfiniteStyle, ::MPOStyle,
        envs::AbstractMPSEnvironments, below::AbstractMPS,
        operator::AbstractMPO, above::AbstractMPS, alg
    )
    # compute eigenvector
...
function compute_rightenvs!(
    ...
    geometry_style = GeometryStyle(envs) & GeometryStyle(below)
        & GeometryStyle(operator) & GeometryStyle(above)
    operator_style = OperatorStyle(operator)
    return compute_rightenvs!(geometry_style, operator_style, envs, below, operator, above, alg)
end

function compute_rightenvs!(::InfiniteStyle, ::MPOStyle,
        envs::AbstractMPSEnvironments, below::AbstractMPS,
        operator::AbstractMPO, above::AbstractMPS, alg
    )
    # compute eigenvector
...
function TensorKit.normalize!(::InfiniteStyle, ::MPOStyle,
        envs::AbstractMPSEnvironments, below::AbstractMPS, operator::AbstractMPO,
        above::AbstractMPS
    )
...
# -----------------------------------
function initialize_environments(::InfiniteStyle, ::HamiltonianStyle,
        below::AbstractMPS, operator::AbstractMPO,
        above::AbstractMPS = below
    )
...
function compute_leftenvs!(::InfiniteStyle, ::HamiltonianStyle,
        envs::AbstractMPSEnvironments, below::AbstractMPS,
        operator::AbstractMPO, above::AbstractMPS, alg
    )
...
function left_cyclethrough!(
    ...
    geometry_style = GeometryStyle(below) &
        GeometryStyle(operator) & GeometryStyle(above)
    operator_style = OperatorStyle(operator)
    return left_cyclethrough!(geometry_style, operator_style, index, GL,
        below, operator, above)
end
function left_cyclethrough!(
        ::InfiniteStyle, ::HamiltonianStyle,
...
function right_cyclethrough!(
    ...
    geometry_style = GeometryStyle(below) &
        GeometryStyle(operator) & GeometryStyle(above)
    operator_style = OperatorStyle(operator)
    return right_cyclethrough!(geometry_style, operator_style, index, GR,
        below, operator, above)
end
function right_cyclethrough!(
        geometry_style::GeometryStyleType, operator_style::OperatorStyleType,
        index::Int, GR, below::AbstractMPS, operator::AbstractMPO,
        above::AbstractMPS = below
    )
    # TODO: efficient transfer matrix slicing for large unitcells
...
function transfer_leftenv!(
    ...
    geometry_style = GeometryStyle(envs) & GeometryStyle(below)
        & GeometryStyle(operator) & GeometryStyle(above)
    operator_style = OperatorStyle(operator)
    return transfer_leftenv!(geometry_style, operator_style, envs, below,
        operator, above, site)
end
function transfer_leftenv!(
        ::InfiniteStyle, ::OperatorStyle,
...
function transfer_rightenv!(
    ...
    geometry_style = GeometryStyle(envs) & GeometryStyle(below)
        & GeometryStyle(operator) & GeometryStyle(above)
    operator_style = OperatorStyle(operator)
    return transfer_rightenv!(geometry_style, operator_style, envs, below,
        operator, above, site)
end
function transfer_rightenv!(
        ::InfiniteStyle, ::OperatorStyle,
