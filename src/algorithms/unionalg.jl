...
function changebonds(state::AbstractMPS, H, alg::UnionAlg, envs = environments(state, H))
    state, envs = changebonds(state, H, alg.alg1, envs)
    state, envs = changebonds(state, H, alg.alg2, envs)
    return state, envs
end

function find_groundstate(state::AbstractMPS, H, alg::UnionAlg, envs::AbstractMPSEnvironments = environments(state, H))
